...
namespace TomLonghurst.RedisClient
{
    ...
    internal ref struct ByteSpanReader
    {
        ...
        public ByteSpanReader(ReadOnlySequence<byte> buffer)
        {
            _buffer = buffer;
        ...
        internal static int FindNext(ByteSpanReader reader, byte value) // very deliberately not ref; want snapshot
        {
            int totalSkipped = 0;
            do
            {
        ...
        internal static int FindNextCrLf(ByteSpanReader reader) // very deliberately not ref; want snapshot
        {
            ...
            do
            {
                ...
                int found = span.IndexOfLineTerminator();
                if (found >= 0)
                {
        ...
        //internal static bool HasBytes(ByteSpanReader reader, int count) // very deliberately not ref; want snapshot
        //{
        //    if (count < 0) throw new ArgumentOutOfRangeException(nameof(count));
        //    do
        //    {
