...
namespace TomLonghurst.RedisClient
{
    internal readonly struct RawResult
    {
        ...
        internal ref struct Tokenizer
        {
            ...
            private ByteSpanReader _value;

            public Tokenizer(in ReadOnlySequence<byte> value)
            {
                _value = new ByteSpanReader(value);
                Current = default;
            }
            ...
            public bool MoveNext()
            {
                ...
                int end = ByteSpanReader.FindNext(_value, terminator);
                if (end < 0)
                {
...
//        }
        internal bool StartsWith(byte[] expected)
        {
            ...
            if (rangeToCheck.IsSingleSegment)
            {
                return rangeToCheck.First.Span.SequenceEqual(expected);
            }
